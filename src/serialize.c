...
{
    return (stream && stream->write_char && stream->write_buffer && stream->print_dbg);
}
...
void serialize_scene(tt_serializer_t* stream, uint8_t scene_num, scene_state_t* scene, char (*text)[32][32])
{
    ...
    for (int l = 0; l < SCENE_TEXT_LINES; l++)
    {
        ...
        if (line_length > 0)
        {
            stream->write_buffer(stream->data, (uint8_t*)((*text)[l]), line_length);
